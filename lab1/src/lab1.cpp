...
int main() {
	...
	El *LA, *LB, *LD, *LC, *LE, *LTmp, *p;
	bool bit_a[N - 1] { }, bit_b[N - 1] { }, bit_c[N - 1] { }, bit_d[N - 1] { },
			bit_e[N - 1] { }, end;
	...
	cout.fill('-');
	cout << endl << '+' << setw(20) << '+' << setw(34) << '+' << setw(18) << '+'
			<< endl;

	cout.fill(' ');
	cout << '|' << setw(34) << " Способ хранения |" << setw(43)
			<< " Множество E |" << setw(17) << " Время обработки |" << endl;

	cout.fill('-');
	cout << '+' << setw(20) << '+' << setw(34) << '+' << setw(18) << '+'
			<< endl;

	cout.fill(' ');
	cout << '|' << setw(27) << " Массив const | ";
